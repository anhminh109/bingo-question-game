...
<html lang="vi">
    ...
    <body>
        <div class="app-shell">
            ...
            <% if (saved) { %>
            <div class="notice success">Đã lưu toàn bộ câu hỏi vào database.</div>
            <% } %>
            <% if (reset) { %>
            <div class="notice warning">Đã reset nội dung 25 câu hỏi về rỗng.</div>
            <% } %>
            ...
            <main class="setting-layout">
                <form class="question-form" action="<%= request.getContextPath() %>/save-questions" method="post">
                    <div class="question-list">
                        ...
                        %>
                        <label class="question-item" for="question_<%= i %>">
                            <span>Ô <%= i %></span>
                    ...
                    <div class="form-actions">
                        <button class="button save" type="submit">Lưu câu hỏi</button>
                    </div>
                </form>

                <form class="reset-form" action="<%= request.getContextPath() %>/reset-questions" method="post"
                      onsubmit="return confirm('Bạn có chắc muốn xóa nội dung của 25 câu hỏi?');">
                    <button class="button danger" type="submit">Reset câu hỏi về rỗng</button>
                </form>
            </main>
        </div>
    </body>
